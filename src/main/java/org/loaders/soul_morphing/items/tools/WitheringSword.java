...
public class WitheringSword extends SwordItem {
    ...
    public WitheringSword() {
        super(WITHERING_MATERIAL,
                1,
                -3.2F,
                new Properties()
                        .setId(ResourceKey.create(ITEMS.getRegistryKey(), ResourceLocation.fromNamespaceAndPath(MODID, "withering_sword")))
        );
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide) {
            for (int i = 0; i <= 3; i++) {
                AreaEffectCloud flame = new AreaEffectCloud(level, player.getX(), player.getY(), player.getZ());
